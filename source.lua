...
local isStudio
local website = "https://github.com/emptier-lab"
...
local velvetUI = isStudio and script.Parent:WaitForChild("velvet UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		velvet:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	velvetUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(velvetUI)
	velvetUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	velvetUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == velvetUI.Name and Interface ~= velvetUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "velvet-Old"
		end
	end
elseif not isStudio then
...
velvetUI.Enabled = false
velvetUI.SmartWindow.Visible = false
velvetUI.Notifications.Template.Visible = false
velvetUI.DisplayOrder = 1000000000

local Main : Frame = velvetUI.SmartWindow
local Dragger = Main.Drag
local dragBar = velvetUI.Drag
...
local Notifications = velvetUI.Notifications
...
-- 	for FlagName, Flag in pairs(velvet.Flags) do
...
-- 		else
-- 			notified = true
-- 			velvet:Notification({Title = "Config Error", Content = "velvet was unable to load or find '"..FlagName.. "'' in the current script.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		velvet:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using velvet Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		velvet:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using velvet Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(velvet.Flags) do
...
function velvet:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function velvet:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "velvet UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "velvet",
		LoadingSubtitle = "by velvet Softworks",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "velvet" and Release or "velvet UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(velvetUI.MobileSupport, velvetUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "velvet/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "velvet/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".velvet") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".velvet"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".velvet", FoundKey)
						end
						velvet:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			...
			DiscordInvite = "noinvitelink" -- The disvord invite link. Do not include the link so for example if my invite was discord.gg/velvet I would put velvet
		}, HomeTabSettings or {})
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "velvet - Debugging Mode" break end
		...
		-- Stolen From Sirius Stuff Begins Here :sob:
		...
		-- Stolen From Sirius Stuff ends here :happy:
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("velvet | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("velvet | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- velvet.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					velvet.Options[Flag] = SliderV
				end

				velvetUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = velvet.ThemeGradient
					Slider.Main.UIStroke.color.Color = velvet.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("velvet | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				velvetUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = velvet.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = velvet.ThemeGradient
				end)

				if Flag then
					velvet.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("velvet | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					velvet.Options[Flag] = BindV
				end

				-- velvet.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("velvet | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("velvet | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- velvet.Flags[DropdownSettings.Flag] = DropdownSettings
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("velvet | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					velvet.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = velvet:SaveConfig(inputPath)
					if not success then
						velvet:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = velvet:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = velvet:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = velvet:LoadConfig(selectedConfig)
					if not success then
						velvet:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = velvet:SaveConfig(selectedConfig)
					if not success then
						velvet:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(velvet.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(velvet.Folder .. "/settings/autoload.txt")
			...
			if isfile(velvet.Folder .. "/settings/autoload.txt") then
				local name = readfile(velvet.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if velvet.Options[Flag] then
						velvet.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if velvet.Options[Flag] and data.value then
						velvet.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if velvet.Options[Flag] and data.text and type(data.text) == "string" then
						velvet.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if velvet.Options[Flag] and data.value then
						velvet.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if velvet.Options[Flag] and data.color then
						velvet.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "velvetInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "velvetInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "velvetInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						velvet.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						velvetUI.ThemeRemote.Value = not velvetUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						velvet.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						velvetUI.ThemeRemote.Value = not velvetUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						velvet.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						velvetUI.ThemeRemote.Value = not velvetUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				velvet.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				velvet.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function velvet:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = velvet.Folder .. "/settings/" .. Path .. ".velvet"

			local data = {
				objects = {}
			}

			for flag, option in next, velvet.Options do
		...
		function velvet:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = velvet.Folder .. "/settings/" .. Path .. ".velvet"
		...
		function velvet:LoadAutoloadConfig()
			if isfile(velvet.Folder .. "/settings/autoload.txt") then
				...
				local success, err = velvet:LoadConfig(name)
				if not success then
					return velvet:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function velvet:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(velvet.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".velvet" then
					local pos = file:find(".velvet", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			velvetUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			velvetUI.MobileSupport.Visible = false
	...
	velvetUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		velvetUI.MobileSupport.Visible = false
...
if isStudio then
	local Window = velvet:CreateWindow({
		Name = "velvet Client - velvet Hub | Blade Ball",
		Subtitle = "by velvet Softworks",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "velvet Client (velvet Hub)",
		LoadingSubtitle = "Loading script for Blade Ball",
		KeySystem = true,
		KeySettings = {
			Title = "velvet Client | Key System",
			Subtitle = "Blade Ball",
			Note = "Please Enter Your Key To Use velvet Client",
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "velvet's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = velvet:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
